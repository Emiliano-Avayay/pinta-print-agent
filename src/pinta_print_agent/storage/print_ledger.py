...
class PrintLedger:
    def __init__(self, db_path: Path):
        # The ledger is owned serially by AgentWorker; disabling SQLite's creator-thread
        # guard permits constructing dependencies before starting that worker.
        Path(db_path).parent.mkdir(parents=True, exist_ok=True); self.connection=sqlite3.connect(str(db_path), check_same_thread=False); self.connection.execute("PRAGMA journal_mode=WAL")
