...
// Health has its own protocol version; it is not the PrintJob payload schema.
...
export class HttpPrintApi implements PrintApi {
  ...
  async health() { const r = await this.request('/api/print-agent/health'); if (r.status !== 200) throw new HttpError('PROTOCOL', `unexpected health HTTP ${r.status}`); const x = await this.json(r) as Record<string, unknown>; if (x.status !== 'ok' || x.schema_version !== HEALTH_SCHEMA_VERSION || x.location_id !== this.config.locationId) throw new HttpError('PROTOCOL', 'invalid health response'); }
